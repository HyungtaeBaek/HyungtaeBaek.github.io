...
<span style="display: block; text-align: center; margin: 5px 0;">$\displaystyle M=(N_{1}+\cdots+N_{n})_{w},$</span>
where, for each $i=1,\dots,n$, $N_{i}$ is a $w$-secondary submodule of $M$ and $P_{i}=\sqrt{\mathrm{Ann}(N_{i})}$.
...
Moreover, if
<span style="display: block; text-align: center; margin: 5px 0;">$\displaystyle M=(N_{1}\oplus\cdots\oplus N_{n})_{w},$</span>
where each $N_{i}$ is a $w$-secondary submodule of $M$ and the ideals $P_{i}=\sqrt{\mathrm{Ann}(N_{i})}$ are pairwise distinct, then $M$ is called <i>strongly $w$-representable</i>.
